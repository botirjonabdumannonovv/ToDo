    ...
    public TodoValidator()
    {
        RuleSet(EntityEvent.OnCreate.ToString(),
            () =>
            {
                ...
                RuleFor(todo => todo.Notes).MinimumLength(3).MaximumLength(128)
                    .When(todo => !string.IsNullOrEmpty(todo.Notes));
        ...
        RuleSet(
            ...
            {
                RuleFor(todo => todo.Title).NotEmpty().MinimumLength(3).MaximumLength(128);
                RuleFor(todo => todo.Notes).MinimumLength(3).MaximumLength(1024)
                    .When(todo => !string.IsNullOrEmpty(todo.Notes));
                RuleFor(todo => todo.DueTime).GreaterThan(DateTimeOffset.Now);
                RuleFor(todo => todo.ReminderTime).GreaterThan(DateTimeOffset.Now); 
            });
